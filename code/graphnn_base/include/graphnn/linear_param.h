...
public:
        ...
        LinearParam(std::string _name, size_t _input_size, size_t _output_size, BiasOption _bo = BiasOption::BIAS)
            : LinearParam<mode, Dtype>(_name, _input_size, _output_size, 0, 1.0 / sqrt(output_size), _bo, false) {}
        
		LinearParam(std::string _name, size_t _input_size, size_t _output_size, Dtype mean, Dtype std, BiasOption _bo = BiasOption::BIAS, bool is_init=false)
		...
		virtual void Reset(Dtype mean, Dtype std, bool is_init)
        {
            //this->p["weight"]->value.SetRandN(mean, std, input_size, output_size);
			if(is_init)
				this->p["weight"]->value.Zeros(input_size, output_size);
			else
				this->p["weight"]->value.SetRandN(mean, std, input_size, output_size);
